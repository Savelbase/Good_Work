package com.rm.toolkit.comment.query.util;

import com.rm.toolkit.comment.query.event.Event;
import com.rm.toolkit.comment.query.event.EventPayload;
import com.rm.toolkit.comment.query.exception.ProjectionException;
import com.rm.toolkit.comment.query.model.Versionable;
...
public class ProjectionUtil {
    ...
    public void validateEvent(Event<? extends EventPayload> event, String entityId, Integer entityCurrentVersion) {
        ...
        if (!userIdFromEvent.equals(entityId)) {
            ...
            throw new ProjectionException(String.format("id сущности %s и entityId id события %s не сопадают",
                    entityId, userIdFromEvent));
        }

        Integer entityVersionFromEvent = event.getVersion();
        if (entityVersionFromEvent != (entityCurrentVersion + 1)) {
            log.error("Версия события {} не подходит к сущности с версией {}",
